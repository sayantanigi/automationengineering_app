...
function stringToHsl(color: Color) {
    let arrayFormat = color.replace('hsl(', '').replace(')', '').replace(/%/g, '').split(',');
...
function getColorDistance(color1: Color, color2: Color) {
    ...
    while (true) {
        ...
        if (lastColor.h == color2Format.h && lastColor.s == color2Format.s && lastColor.l == color2Format.l) {
            break;
        }

        if (lastColor.h != color2Format.h) {
            resTemplate.h = lastColor.h > color2Format.h ? lastColor.h - 1 : lastColor.h + 1
        }
        if (lastColor.s != color2Format.s) {
            resTemplate.s = lastColor.s > color2Format.s ? lastColor.s - 1 : lastColor.s + 1
        }


        if (lastColor.l != color2Format.l) {
            resTemplate.l = lastColor.l > color2Format.l ? lastColor.l - 1 : lastColor.l + 1
        }
